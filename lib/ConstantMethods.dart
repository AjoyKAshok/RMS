...
List<String> logtime = [];
String appversionnumber = "v3.3.5";
String logoutmessage;
bool showUploadDDialog= false;
...
createlog(message, status) async {
  ...
  if (logreport.length < 500) {
    ...
    await smtpExample(
        'automatic >500 Log details for empid ${DBrequestdata.receivedempid}');
    logreport = [];
    logreportstatus = [];
    logtime = [];
...
String nblfile = Uri.encodeFull(
    "https://test.rhapsody.ae/nbl_file/${NBLDetData.fileurl.last}");
      ...
      connectivityBuilder: (BuildContext context,
...

        {
          ...
          Future.delayed(const Duration(seconds: 1), () async {
            ...
            {
              print("_onlineNotificationStatemessage0");
            }

          });
        } else {

          // saveSharedConnection(false);
...
adddataforsync(url, body, comment) async {
  ...
  await saveDataForSync(url,body,comment,addDataForSync);
  return;
