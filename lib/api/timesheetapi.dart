...
Future getTimeSheetdaily() async {
  ...
  print(timesheetfm);
  try {
    http.Response tsresponse = await http.post(
      TSurl,
      headers: {
      ...
      body: currentuser.roleid == 6
          ? jsonEncode(Timesheetrequest)
          : jsonEncode(timesheetfm),
    );
...
Future CDETimeSheetApproval() async {
  ...
  http.Response addapprove = await http.post(
    CDEApproveTimeSheet,
    headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
