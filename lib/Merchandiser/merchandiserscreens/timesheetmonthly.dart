                      ...
                      children: [
                        ...
                        GestureDetector(
                            ...
                            if (alldateselected) {
                              setState(() {
                                ...
                                for (int i = 0; i < dataofdates.length; i++) {
                                  int index = listOfDates.indexOf(
                                      "${DateFormat('MMMMd').format(DateTime.parse(dataofdates[i]))}");
                          ...
                          child: Icon(
                            ...
                            color:
                                alldateselected == true ? orange : Colors.grey,
                            size: 30,
                          ),
                        ),
                      ],
                  ...
                  ? listOfDates.length > 28
                      ...
                      : TMmonthly1.day29.length != 0
                          ? Column(
                              children: [
                                Customtimesheetheader(index: 28),
                                CustomLeaveTypeWidget(
                                    inputdata: TMmonthly1.day29),
                              ],
                            )
                          : SizedBox()
                  : SizedBox(),
...
class CustomTimesheetWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    //List<String> split_outlet = [];
    //List<String> split_checkin = [];
    //List<String> split_checkout = [];
    ...
    for (int i = 0; i < inputdata.length; i++) {
      ...
      // split_outlet.add(decodeddata['split_Outlet']);
      // split_checkin.add(decodeddata['split_CheckIn']);
      //split_checkout.add(decodeddata['split_CheckOut']);
    }
                ...
                return Column(
                  children: [
                    Table(
                      ...
                      children: [
                        TableRow(children: [
                          if (outlet[index].contains('(split)') == true)
                            Padding(
                              padding: const EdgeInsets.only(bottom: 5.0),
                              child: Text(
                                outlet[index],
                          ...
                          if (outlet[index].contains('(split)') == false)
                            Padding(
                              padding: const EdgeInsets.only(bottom: 5.0),
                              child: Text(
                          ...
                          /*  Padding(
                            padding: const EdgeInsets.only(bottom: 5.0),
                            child: Text(
                              split_outlet[index],softWrap: true,
                            ),
                          ),
                          Center(
                            child: Text(
                    ...
                    // else
                    //  index ==outlet.length-2 ?SizedBox():Divider(color: Colors.grey,)
                  ],
                );
              })
        ],
