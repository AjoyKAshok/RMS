...
String todayjp_table="todayjp",todayskipped_table="todayskipped",todayvisited_table="todayvisited";
String journeyplanweekly_table="journeyplanweekly",skjourneyplanweekly_table="skjourneyplanweekly",vstjourneyplanweekly_table="vstjourneyplanweekly";
String nbldetaildata_table="nbldetaildata",availabilitydetdata_table="availabilitydetdata",visibilitydetdata_table="visibilitydetdata",
    shareofshelfdetdata_table="shareofshelfdetdata",planodetaildata_table="planodetaildata",promodetaildata_table="promodetaildata",
    checklistdetaildata_table="checklistdetaildata",expiryproductsdata_table ="expiryproductsdata";
String alljpoutletschart_table="alljpoutletschart",alljpoutletsEchart_table="alljpoutletsEchart",alljpoutlets_table="alljpoutlets";
...
class DatabaseHelper {
  ...
  Future<int> insertActivityData(DBActivitiesModel dbModel,String table) async {
    Database db = await this.database;
    var result = await db.insert(table, dbModel.toMap());
    return result;
  }
  Future<int> insertDataForSync(DBAddDataServerModel dbModel,String table) async {
    Database db = await this.database;
    var result = await db.insert(table, dbModel.toMap());
    return result;
  }
...
print("delted value ..$i");
  ...
  updateActivityData(DBActivitiesModel dbModel,String table)
  async {
    Database db = await this.database;
    await db.update(
      '$table',
...
// return data;
  ...
  Future <List<String>> getList(String table)
  async {
    List<DBModel> noteListFuture = await getDataList(table);
    List<String>data = [];
    List<DBModel> bookList= noteListFuture;
  ...
  getActivityList(String table,String timesheet)
  async {
    Database db = await this.database;
    // raw query
    List<Map> result = await db.rawQuery('SELECT $firstColumn FROM $table WHERE $timesheetID=?', ['$timesheet']);
...
getDataForSyncList_(String table)
async {
...
saveActivitiesDataIntoDB(String timesheet,String data,String table)
async {
  DatabaseHelper helper = DatabaseHelper();
...
saveDataForSync(String url,String body,String comment,String table)
async {
  DatabaseHelper helper = DatabaseHelper();
...
///save list data
saveListDataIntoDb(String data,String table)
async {
  DatabaseHelper helper = DatabaseHelper();
  DBModel dbModel;
  dbModel = new DBModel(data);
...
getActivityData(String table,String timesheet)
async {
  DatabaseHelper helper = DatabaseHelper();
